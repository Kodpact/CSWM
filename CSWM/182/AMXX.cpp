...
C_DLLEXPORT int GetEntityAPI2(DLL_FUNCTIONS *GameFunctionsPtr, int *InterfaceVersion)
{
	...
	memcpy(GameFunctionsPtr, &g_EntityAPI_Table, sizeof(DLL_FUNCTIONS));
	g_pFunctionTable = GameFunctionsPtr;
	return TRUE;
}

C_DLLEXPORT int GetEntityAPI2_Post(DLL_FUNCTIONS *GameFunctionsPtr, int *InterfaceVersion)
{
	if (!GameFunctionsPtr)
	{
		LOG_ERROR(PLID, "GetEntityAPI2_Post Called Without Valid DLL_FUNCTIONS.");
		return FALSE;
	}
	else if (*InterfaceVersion != INTERFACE_VERSION)
	{
	...
	memcpy(GameFunctionsPtr, &g_EntityAPI_Post_Table, sizeof(DLL_FUNCTIONS));
	g_pFunctionTable_Post = GameFunctionsPtr;
	return TRUE;
}

C_DLLEXPORT int GetEngineFunctions(enginefuncs_t *EngineFuncsPtr, int *InterfaceVersion)
{
	if (!EngineFuncsPtr)
	{
		LOG_ERROR(PLID, "GetEngineFunctions Called Without Valid enginefuncs_t.");
		return FALSE;
	}
	...
	memcpy(EngineFuncsPtr, &g_EngineFuncs_Table, sizeof(enginefuncs_t));
	g_pengfuncsTable = EngineFuncsPtr;
	return TRUE;
}
