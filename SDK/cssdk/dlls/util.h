...
#define INDEXENT(x) (*g_engfuncs.pfnPEntityOfEntIndex)(x)
#endif // INDEXENT
...
inline void EMIT_SOUND(edict_t *entity, int channel, const char *sample, float vol = VOL_NORM, float attn = ATTN_NORM, float pitch = PITCH_NORM)
{
	EMIT_SOUND_DYN2(entity, channel, sample, vol, attn, 0, pitch);
}
